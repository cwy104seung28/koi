...
$colname_RecmainteaTitle = "-1";
if (isset($_GET['d_id'])) {
    $colname_RecmainteaTitle = $_GET['d_id'];
}

$query_RecmainteaTitle = "SELECT * FROM data_set WHERE d_id = :d_id";
$RecmainteaTitle = $conn->prepare($query_RecmainteaTitle);
$RecmainteaTitle->bindParam(':d_id', $colname_RecmainteaTitle, PDO::PARAM_INT);
$RecmainteaTitle->execute();
$row_RecmainteaTitle = $RecmainteaTitle->fetch();
$totalRows = $RecmainteaTitle->rowCount();
...
$RecImage->bindParam(':file_d_id', $colname_RecmainteaTitle, PDO::PARAM_INT);
$RecImage->execute();
$row_RecImage = $RecImage->fetch();
$totalRows_RecImage = $RecImage->rowCount();

$query_RecCover = "SELECT * FROM file_set WHERE file_d_id = :file_d_id AND file_type = 'mainteaTitleCover'";
$RecCover = $conn->prepare($query_RecCover);
$RecCover->bindParam(':file_d_id', $colname_RecmainteaTitle, PDO::PARAM_INT);
$RecCover->execute();
$row_RecCover = $RecCover->fetch();
$totalRows_RecCover = $RecCover->rowCount();

$query_RecCenterCover = "SELECT * FROM file_set WHERE file_d_id = :file_d_id AND file_type = 'mainteaTitleCenterCover'";
$RecCenterCover = $conn->prepare($query_RecCenterCover);
$RecCenterCover->bindParam(':file_d_id', $colname_RecmainteaTitle, PDO::PARAM_INT);
...
$RecFile->bindParam(':file_d_id', $colname_RecmainteaTitle, PDO::PARAM_INT);
$RecFile->execute();
                                                ...
                                                <tr>
                                                    ...
                                                    <td width="532">
                                                        <input name="d_id" type="hidden" id="d_id"
                                                            value="<?php echo $row_RecmainteaTitle['d_id']; ?>" />
                                                        <input name="d_title" type="text" class="table_data"
                                                            id="d_title"
                                                            value="<?php echo $row_RecmainteaTitle['d_title']; ?>" size="80">
                                                    </td>
                                                ...
                                                <tr>
                                                    ...
                                                    <td width="532">
                                                        <input name="d_title_en" type="text" class="table_data"
                                                            id="d_title_en"
                                                            value="<?php echo $row_RecmainteaTitle['d_title_en']; ?>"
                                                            size="80">
                                                    </td>
                                                    <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                                                </tr>
                                                ...
                                                <tr>
                                                    ...
                                                    <td width="532">
                                                        <select name="d_active" class="table_data" id="d_active">
                                                            <option value="1" <?php if (!(strcmp(1, $row_RecmainteaTitle['d_active']))) {
                                                                echo "selected";
                                                            } ?>>顯示</option>
                                                            <option value="0" <?php if (!(strcmp(0, $row_RecmainteaTitle['d_active']))) {
                                                                echo "selected";
                                                            } ?>>不顯示</option>
                                                        </select>
                                                    </td>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
    ...
    // Cover
    $image_result = image_process($conn, $_FILES['imageCover'], $_REQUEST['imageCover_title'], $menu_is, "add", $imagesSize['mainteaTitleCover']['IW'], $imagesSize['mainteaTitleCover']['IH']);
    ...
    for ($j = 1; $j < count($image_result); $j++) {
        ...
        $stat->bindParam(':file_link2', $image_result[$j][2], PDO::PARAM_STR);
        $stat->bindParam(':file_link3', $image_result[$j][3], PDO::PARAM_STR);
        $stat->bindParam(':file_type', $type = 'mainteaTitleCover', PDO::PARAM_STR);
        $stat->bindParam(':file_d_id', $_POST['d_id'], PDO::PARAM_INT);
        $stat->bindParam(':file_title', $image_result[$j][4], PDO::PARAM_STR);
        $stat->bindParam(':file_show_type', $image_result[$j][5], PDO::PARAM_INT);
        $stat->execute();
    ...
    // CenterCover
    $image_result = image_process($conn, $_FILES['imageCenterCover'], $_REQUEST['imageCenterCover_title'], $menu_is, "add", $imagesSize['mainteaTitleCenterCover']['IW'], $imagesSize['mainteaTitleCenterCover']['IH']);
    ...
    for ($j = 1; $j < count($image_result); $j++) {
        ...
        $stat->bindParam(':file_link2', $image_result[$j][2], PDO::PARAM_STR);
        $stat->bindParam(':file_link3', $image_result[$j][3], PDO::PARAM_STR);
        $stat->bindParam(':file_type', $type = 'mainteaTitleCenterCover', PDO::PARAM_STR);
        $stat->bindParam(':file_d_id', $_POST['d_id'], PDO::PARAM_INT);
        $stat->bindParam(':file_title', $image_result[$j][4], PDO::PARAM_STR);
        $stat->bindParam(':file_show_type', $image_result[$j][5], PDO::PARAM_INT);
        $stat->execute();
    ...
    if ($ifFile) {
        $file_result = file_process($conn, "mainteaTitle", "add");
    ...
    $updateGoTo = "mainteaTitle_list.php";
    if (isset($_SERVER['QUERY_STRING'])) {
