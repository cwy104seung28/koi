...
$Recmenu = $conn->query($query_limit_Recmenu);
...
$all_Recmenu = $conn->query($query_Recmenu);
$totalRows = $all_Recmenu->rowCount();
...
if ($G_changeSort == 1 || $G_delchangeSort == 1) {
    ...
    do {
        if ($row_Recmenu['d_sort'] == 0) {} else if ($row_Recmenu['d_id'] == $_GET['now_d_id']) {
            ...
            $stat->bindParam(':d_id', $row_Recmenu['d_id'], PDO::PARAM_INT);
            $stat->execute();
    ...
    } while ($row_Recmenu = $Recmenu->fetch());
                            ...
                            <table width="100%" border="0" cellpadding="0" cellspacing="0" bgcolor="#E1E1E1" class="list_title_table">
                                <tr>
                                    <td width="739" align="right" class="page_display">
                                        <!-------顯示頁選擇與分頁設定開始---------->
                                        <?php displayPages($pageNum, $queryString_Recmenu, $totalPages_Recmenu, $totalRows, $currentPage); ?>
                                        <!-------顯示頁選擇與分頁設定結束---------->
                                    </td>
