...
$colname_RecstoreNum = "-1";
if (isset($_GET['d_id'])) {
    $colname_RecstoreNum = $_GET['d_id'];
}

$query_RecstoreNum = "SELECT * FROM data_set WHERE d_id = :d_id";
$RecstoreNum = $conn->prepare($query_RecstoreNum);
$RecstoreNum->bindParam(':d_id', $colname_RecstoreNum, PDO::PARAM_INT);
$RecstoreNum->execute();
$row_RecstoreNum = $RecstoreNum->fetch();
$totalRows = $RecstoreNum->rowCount();
...
$RecImage->bindParam(':file_d_id', $colname_RecstoreNum, PDO::PARAM_INT);
$RecImage->execute();
$row_RecImage = $RecImage->fetch();
$totalRows_RecImage = $RecImage->rowCount();

$query_RecCover = "SELECT * FROM file_set WHERE file_d_id = :file_d_id AND file_type = 'storeNumCover'";
$RecCover = $conn->prepare($query_RecCover);
$RecCover->bindParam(':file_d_id', $colname_RecstoreNum, PDO::PARAM_INT);
$RecCover->execute();
$row_RecCover = $RecCover->fetch();
$totalRows_RecCover = $RecCover->rowCount();

$query_RecCenterCover = "SELECT * FROM file_set WHERE file_d_id = :file_d_id AND file_type = 'storeNumCenterCover'";
$RecCenterCover = $conn->prepare($query_RecCenterCover);
$RecCenterCover->bindParam(':file_d_id', $colname_RecstoreNum, PDO::PARAM_INT);
...
$RecFile->bindParam(':file_d_id', $colname_RecstoreNum, PDO::PARAM_INT);
$RecFile->execute();
                                            ...
                                            <table width="100%" border="0" cellspacing="3" cellpadding="5">
                                                <tr>
                                                    <td width="200" align="center" bgcolor="#e5ecf6"
                                                        class="table_col_title">數字(中文頁面)</td>
                                                    <td width="532">
                                                        <input name="d_id" type="hidden" id="d_id"
                                                            value="<?php echo $row_RecstoreNum['d_id']; ?>" />
                                                        <input name="d_title" type="text" class="table_data"
                                                            id="d_title"
                                                            value="<?php echo $row_RecstoreNum['d_title']; ?>" size="80">
                                                    </td>
                                                    <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                                                </tr>
                                                <tr>
                                                    <td width="200" align="center" bgcolor="#e5ecf6"
                                                        class="table_col_title">數字(英文頁面)</td>
                                                    <td width="532">
                                                        <input name="d_title_en" type="text" class="table_data"
                                                            id="d_title_en"
                                                            value="<?php echo $row_RecstoreNum['d_title_en']; ?>" size="80">
                                                    </td>
                                                    <td width="250" bgcolor="#e5ecf6">&nbsp;</td>
                                                </tr>
                                                <tr>
                                                ...
                                                <tr>
                                                    ...
                                                    <td width="532">
                                                        <select name="d_active" class="table_data" id="d_active">
                                                            <option value="1" <?php if (!(strcmp(1, $row_RecstoreNum['d_active']))) {
                                                                echo "selected";
                                                            } ?>>顯示</option>
                                                            <option value="0" <?php if (!(strcmp(0, $row_RecstoreNum['d_active']))) {
                                                                echo "selected";
                                                            } ?>>不顯示</option>
                                                        </select>
                                                    </td>
...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
    ...
    // Cover
    $image_result = image_process($conn, $_FILES['imageCover'], $_REQUEST['imageCover_title'], $menu_is, "add", $imagesSize['storeNumCover']['IW'], $imagesSize['storeNumCover']['IH']);
    ...
    for ($j = 1; $j < count($image_result); $j++) {
        ...
        $stat->bindParam(':file_link2', $image_result[$j][2], PDO::PARAM_STR);
        $stat->bindParam(':file_link3', $image_result[$j][3], PDO::PARAM_STR);
        $stat->bindParam(':file_type', $type = 'storeNumCover', PDO::PARAM_STR);
        $stat->bindParam(':file_d_id', $_POST['d_id'], PDO::PARAM_INT);
        $stat->bindParam(':file_title', $image_result[$j][4], PDO::PARAM_STR);
        $stat->bindParam(':file_show_type', $image_result[$j][5], PDO::PARAM_INT);
        $stat->execute();
    ...
    // CenterCover
    $image_result = image_process($conn, $_FILES['imageCenterCover'], $_REQUEST['imageCenterCover_title'], $menu_is, "add", $imagesSize['storeNumCenterCover']['IW'], $imagesSize['storeNumCenterCover']['IH']);
    ...
    for ($j = 1; $j < count($image_result); $j++) {
        ...
        $stat->bindParam(':file_link2', $image_result[$j][2], PDO::PARAM_STR);
        $stat->bindParam(':file_link3', $image_result[$j][3], PDO::PARAM_STR);
        $stat->bindParam(':file_type', $type = 'storeNumCenterCover', PDO::PARAM_STR);
        $stat->bindParam(':file_d_id', $_POST['d_id'], PDO::PARAM_INT);
        $stat->bindParam(':file_title', $image_result[$j][4], PDO::PARAM_STR);
        $stat->bindParam(':file_show_type', $image_result[$j][5], PDO::PARAM_INT);
        $stat->execute();
    ...
    if ($ifFile) {
        $file_result = file_process($conn, "storeNum", "add");
