...
$maxRows_Recrecruit = 10;
$pageNum = 0;
if (isset($_GET['pageNum'])) {
    $pageNum = $_GET['pageNum'];
}
$startRow_Recrecruit = $pageNum * $maxRows_Recrecruit;

$query_Recrecruit = "SELECT * FROM data_set WHERE d_class1 = '$menu_is' ORDER BY d_sort ASC, d_date DESC";
$query_limit_Recrecruit = sprintf("%s LIMIT %d, %d", $query_Recrecruit, $startRow_Recrecruit, $maxRows_Recrecruit);
$Recrecruit = $conn->query($query_limit_Recrecruit);
$row_Recrecruit = $Recrecruit->fetch();

if (isset($_GET['totalRows'])) {
    $totalRows = $_GET['totalRows'];
} else {
    $all_Recrecruit = $conn->query($query_Recrecruit);
    $totalRows = $all_Recrecruit->rowCount();
}
$totalPages = ceil($totalRows / $maxRows_Recrecruit) - 1;
...
if ($R_pageNum > $totalPages && $R_pageNum != 0) {
    header("Location:recruit_list.php?pageNum=" . $totalPages);
}
...
if ($G_changeSort == 1 || $G_delchangeSort == 1) {
    ...
    $query_Recrecruit = "SELECT * FROM data_set WHERE d_class1 = 'recruit' ORDER BY d_sort ASC, d_date DESC";
    $Recrecruit = $conn->query($query_Recrecruit);
    $row_Recrecruit = $Recrecruit->fetch();

    do {
        if ($row_Recrecruit['d_sort'] == 0) {} else if ($row_Recrecruit['d_id'] == $_GET['now_d_id']) {
            ...
            $stat->bindParam(':d_id', $row_Recrecruit['d_id'], PDO::PARAM_INT);
            $stat->execute();
    ...
    } while ($row_Recrecruit = $Recrecruit->fetch());
    ...
    if ($G_changeSort == 1) {
        header("Location:recruit_list.php?pageNum=" . $_GET['pageNum'] . "&totalRows=" . $_GET['totalRows']);
    } else if ($G_delchangeSort == 1) {
        header("Location:recruit_list.php?pageNum=" . $_GET['pageNum']);
    }
}
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0">
                    <tr>
                        <td align="left">
                            ...
                            <form action="recruit_process.php" method="post" name="form1" id="form1">
                                ...
                                <table width="100%" border="0" align="center" cellpadding="5" cellspacing="1">
                                    ...
                                    do {
                                        $i++;
                                        $colname_RecImage = "-1";
                                        if (isset($row_Recrecruit['d_id'])) {
                                          $colname_RecImage = $row_Recrecruit['d_id'];
                                        }
                                    ...
                                    <tr <?php if ($i%2==0): ?>bgcolor='#E4E4E4'<?php endif ?>>
                                        <td align="center" class="table_data">
                                            <a href="recruit_edit.php?d_id=<?php echo $row_Recrecruit['d_id']; ?>">
                                                <?php echo $row_Recrecruit['d_date']; ?>
                                            </a>
                                        </td>
                                        <td align="center" class="table_data">
                                            <select name="d_sort" id="d_sort" onchange="changeSort('<?php echo $pageNum; ?>','<?php echo $totalRows; ?>','<?php echo $row_Recrecruit['d_id']; ?>',this.options[this.selectedIndex].value)">
                                                <option value="0" <?php if (!(strcmp(0, $row_Recrecruit[ 'd_sort']))) {echo "selected"; } ?>>至頂</option>
                                                <?php
                                                for($j=1;$j<=($totalRows);$j++) {
                                                    echo "<option value=\"".$j."\" ";
                                                    if (!(strcmp($j, $row_Recrecruit['d_sort']))) {echo "selected=\"selected\"";}
                                                    echo ">".$j."</option>";
                                                }
                                                $_SESSION['totalRows']=$totalRows;
                                                ?>
                                        ...
                                        <!-- <td align="center" class="table_data"><a href="recruit_edit.php?d_id=<?php echo $row_Recrecruit['d_id']; ?>">
...
<script type="text/javascript">
    function changeSort(pageNum, totalRows, now_d_id, change_num) { //v1.0
        ...
        window.location.href = "recruit_list.php?pageNum=" + pageNum + "&totalRows=" + totalRows + "&changeSort=1" + "&now_d_id=" + now_d_id + "&change_num=" + change_num;
    }
</script>