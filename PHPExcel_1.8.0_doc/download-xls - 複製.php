...
if (isset($_REQUEST['outexcel']) && ($_REQUEST['outexcel'] == "1")) {
    ...
    $RecOrder = mysqli_query($connect2data, $query_RecOrder) or die(mysqli_error());
    $row_RecOrder = mysqli_fetch_assoc($RecOrder);
    $totalRows_RecOrder = mysqli_num_rows($RecOrder);
    ...
    if ($totalRows_RecOrder > 0) {
        ...
        $objPHPExcel->getActiveSheet()->getColumnDimension("B")->setWidth(11); //訂單日期
        ...
        $objPHPExcel->getActiveSheet()->getColumnDimension("G")->setWidth(20); //付款方式
		...
		// $objPHPExcel->getActiveSheet()->getStyle("A2")->getFill()->getStartColor()->setRGB('857F7F');
        ...
        do {
            ...
            if (!(strcmp(3, $row_RecOrder['payment']))) {$payment = "信用卡線上刷卡";}
            if (!(strcmp(5, $row_RecOrder['payment']))) {$payment = "自取";}
            ...
            $RecOrderItem = mysqli_query($connect2data, $query_RecOrderItem) or die(mysqli_error());
            $row_RecOrderItem = mysqli_fetch_assoc($RecOrderItem);
            $totalRows_RecOrderItem = mysqli_num_rows($RecOrderItem);
            ...
            //Write cells
            $sheet->setCellValue('A' . $iii, '商品清單')
                ->setCellValue('A' . $iiii, '商品名稱')
                ->setCellValue('B' . $iiii, '售價')
                ->setCellValue('C' . $iiii, '數量')
                ->setCellValue('D' . $iiii, '小計');
            ...
            do {
                $sheet->setCellValue('A' . $j, $row_RecOrderItem['d_name']);
                $sheet->setCellValue('B' . $j, $row_RecOrderItem['d_price1']);
                $sheet->getStyle('B' . $j)->getNumberFormat()->setFormatCode('_-"NT$"* #,##0_ ;_-"NT$"* -#,##0 ;_-"NT$"* "-"_ ;_-@_ ');
                $sheet->setCellValue('C' . $j, $row_RecOrderItem['qty']);
                $sheet->setCellValue('D' . $j, $row_RecOrderItem['subtotal']);
                $sheet->getStyle('D' . $j)->getNumberFormat()->setFormatCode('_-"NT$"* #,##0_ ;_-"NT$"* -#,##0 ;_-"NT$"* "-"_ ;_-@_ ');

                // $sheet->getRowDimension($j)->setRowHeight(20);

                $j++;

            } while ($row_RecOrderItem = mysqli_fetch_assoc($RecOrderItem));


            $jj = $j - 1;

            $sheet->getStyle("A{$iii}:D{$jj}")->getBorders()->getAllBorders()->setBorderStyle(PHPExcel_Style_Border::BORDER_THIN);

            $sheet->getStyle("A{$iii}:D{$jj}")->getFill()->setFillType(PHPExcel_Style_Fill::FILL_SOLID);
			$sheet->getStyle("A{$iii}:D{$jj}")->getFill()->getStartColor()->setRGB('F5F5F5');


            // Rename worksheet
            $objPHPExcel->getActiveSheet()->setTitle(date('Ymd') . '-bsquare全訂單');
        ...
        } while ($row_RecOrder = mysqli_fetch_assoc($RecOrder));

        $rows = mysqli_num_rows($RecOrder);
        if ($rows > 0) {
            mysqli_data_seek($RecOrder, 0);
            $row_RecOrder = mysqli_fetch_assoc($RecOrder);
        }
        ...
        mysqli_set_charset('utf8');//这里要注意，我搞不懂utf8跟utf-8的区别，因此出现乱码，浪费了好多时间
        $Result = mysqli_query($connect2data, $sql);
        ...
        header('Content-Disposition: attachment;filename="' . date('Ymd') . '-bsquare.xls"');
